...
이때 파이썬 인터프리터가 \dotemph{가비지 수집}(garbage collection)을 수행해야 한다.  힙을 채운 모든 객체를 각각 조사하고 객체가 \dotemph{가비지}(garbage, 쓰레기)인지 결정한다. 가비지라면 말 그대로 객체가 더는 쓰일 수 없는 것이다. 이들 가비지 객체는 폐기되고 프로그램은 계속될 수 있다.
